...
Fixes mypy type errors and eliminates `Any` usage in the yt-dlp-utils project.
...
1. Fix all mypy errors reported by `uv run mypy yt_dlp_utils`
...
Place new TypedDict classes in `yt_dlp_utils/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`yt_dlp_utils/typing.py` when shared.
...
1. Run `uv run mypy yt_dlp_utils` and capture all errors.
...
4. Run `uv run mypy yt_dlp_utils` again to verify fixes.
