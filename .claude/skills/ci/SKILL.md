...
If any changed files are under `yt_dlp_utils/` or `tests/`, run the following agents **in order**:
...
- **changelog** - update `CHANGELOG.md` with entries for the changes. After it completes, check if
  ...
  the relevant commit. **Only run when changes affect users**: files under `yt_dlp_utils/`, `tests/`,
...
For example, if a commit contains `yt_dlp_utils/commands/main.py`,
`tests/test_main_command.py`, and `CHANGELOG.md`, the component is determined by
`yt_dlp_utils/commands/main.py` and `tests/test_main_command.py` only.
...
- Changes span unrelated components (e.g. `yt_dlp_utils/media.py` and `.claude/agents/release.md`).
...
For Python files, strip the `yt_dlp_utils/` prefix and replace `/` with `.` (like module imports).

- Python file `yt_dlp_utils/media.py` → `media:`.
- Multiple files under `yt_dlp_utils/commands/` → `commands:`.
- Single command file `yt_dlp_utils/commands/admin.py` → `commands.admin:`.
